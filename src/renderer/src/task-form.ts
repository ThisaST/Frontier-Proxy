...
export interface TaskForm {
  ...
  scheduleSkillsLoad(cwd: string): void
  resetSkills(): void
  reset(): void
  submit(cwd: string): Promise<ProxyTask>
}

export function createTaskForm(ids: TaskFormIds): TaskForm {
  ...
  function renderTaskSkillsField(): void {
    if (!ids.skillsList) return
    const list = byId(ids.skillsList)
    updateTaskSkillsSummary()
    if (!taskSkillsCatalog) { list.replaceChildren(emptyState('No skills scanned yet', 'Choose a project to see the skills Frontier found.')); return }
  ...
  function selectedBenchProviders(): string[] {
    if (!ids.benchProviders) return []
    return [...byId(ids.benchProviders).querySelectorAll<HTMLInputElement>('input:checked')].map((input) => input.value)
  }

  function setRunMode(mode: RunMode): void {
    runMode = mode
    document.querySelectorAll<HTMLElement>(`${ids.root} .run-mode`).forEach((button) => {
  ...
  function renderProviderOptions(): void {
    const select = byId<HTMLSelectElement>(ids.providerOverrideSelect)
  ...
  function renderModelOptions(): void {
    const select = byId<HTMLSelectElement>(ids.modelSelect)
    const custom = byId<HTMLInputElement>(ids.modelCustom)
    const current = select.value
    const overrideId = byId<HTMLSelectElement>(ids.providerOverrideSelect).value
  ...
  function selectedModel(): string | undefined {
    const choice = byId<HTMLSelectElement>(ids.modelSelect).value
    if (choice === '__custom__') return byId<HTMLInputElement>(ids.modelCustom).value.trim() || undefined
  ...
  function selectedModelProvider(): string | undefined {
    const select = byId<HTMLSelectElement>(ids.modelSelect)
    if (select.value === '__custom__') return byId<HTMLSelectElement>(ids.providerOverrideSelect).value || undefined
    return select.selectedOptions[0]?.dataset.providerId || undefined
  }

  function reset(): void {
    byId<HTMLTextAreaElement>(ids.promptInput).value = ''
    byId<HTMLSelectElement>(ids.modelSelect).value = ''
    byId<HTMLInputElement>(ids.modelCustom).value = ''
    byId<HTMLInputElement>(ids.modelCustom).hidden = true
    clearComposerDraft(ids.promptInput)
    setRunMode('single')
  }

  async function submit(cwd: string): Promise<ProxyTask> {
    ...
    const prompt = byId<HTMLTextAreaElement>(ids.promptInput).value
    const task = await window.frontier.createTask({
      prompt,
      cwd,
      mode: byId<HTMLSelectElement>(ids.routingModeSelect).value as CreateTaskInput['mode'],
      preferredProviderId: runMode === 'single' ? byId<HTMLSelectElement>(ids.providerOverrideSelect).value || undefined : undefined,
      ...
      attachments: messageContext(ids.promptInput, prompt),
      skillIds: taskSkillsTouched ? [...taskSkillsSelection] : undefined
    })
    for (const item of composerDraft(ids.promptInput).items) {
      const preview = composerDraft(ids.promptInput).previews.get(item.id)
  ...
  function init(): void {
    ...
    byId<HTMLSelectElement>(ids.modelSelect).addEventListener('change', () => {
      const custom = byId<HTMLInputElement>(ids.modelCustom)
      custom.hidden = byId<HTMLSelectElement>(ids.modelSelect).value !== '__custom__'
  ...
  return {
    ...
    scheduleSkillsLoad: scheduleTaskSkillsLoad,
    resetSkills: resetTaskSkillsState,
    reset,
    submit
  }
}
