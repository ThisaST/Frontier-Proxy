...
- Control-plane translation is CLI-specific and pure: Claude uses `--mcp-config`/`--allowedTools`/`--add-dir`/`--append-system-prompt` (+ `mcp__<server>__*` in allowedTools); Copilot uses `--additional-mcp-config`/`--allow-tool=`/`--deny-tool=` and folds the shared prompt into `promptPrefix`; Codex uses per-invocation `-c 'mcp_servers.<name>={...}'` overrides + `developer_instructions`, aliasing non-simple server names, and `default_tools_approval_mode = "approve"`. SSE is not injected for Codex. OpenCode has no per-run flags: the profile is an inline config document in `OPENCODE_CONFIG_CONTENT` (`mcp`, `skills.paths`, `permission` with Claude-style `Tool(pattern)` → pattern maps and `mcp__<server>__<tool>` → `<server>_<tool>`), plus `promptPrefix`.
